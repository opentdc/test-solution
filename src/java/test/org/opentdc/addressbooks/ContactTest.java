...
public class ContactTest extends AbstractTestClient<AddressbooksService> {
		...
		public void testContactModifications() {
			// create(new ContactModel()) -> _o
			Response _response = webclient.replacePath("/").path(adb.getId()).path(PATH_EL_CONTACT).post(new ContactModel());
			ContactModel _o = _response.readEntity(ContactModel.class);
			...
			_o.setFn("MY_NAME2");
			webclient.type(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON);
			_response = webclient.replacePath("/").path(adb.getId()).path(PATH_EL_CONTACT).path(_o.getId()).put(_o);
			assertEquals("update() should return with status OK", Status.OK.getStatusCode(), _response.getStatus());
			ContactModel _o2 = _response.readEntity(ContactModel.class);
			...
			_response = webclient.replacePath("/").path(adb.getId()).path(PATH_EL_CONTACT).path(_o.getId()).delete();		
			assertEquals("delete() should return with status NO_CONTENT", Status.NO_CONTENT.getStatusCode(), _response.getStatus());
		}
